...
// nada
ZynapseConnection::ZynapseConnection(SpikingGroup *source, NeuronGroup *destination, AurynFloat w_o, TransmitterType transmitter)
...
ZynapseConnection::ZynapseConnection(SpikingGroup *source, NeuronGroup *destination, AurynFloat w_o, AurynFloat sparseness, TransmitterType transmitter)
  : DuplexConnection(source, destination, w_o, sparseness, transmitter)
...
ZynapseConnection::ZynapseConnection(SpikingGroup *source, NeuronGroup *destination, AurynFloat w_o, AurynFloat sparseness, AurynFloat tau_hom, AurynFloat kappa, TransmitterType transmitter, string name)
  : DuplexConnection(source, destination, w_o, sparseness, transmitter, name)
...
void ZynapseConnection::alloc_vectors()
{
  if (dst->get_post_size() == 0) return;

  size_t size = get_nonzero();

  for (int i=0; i<3; i++) {
    layers[i] = gsl_vector_float_calloc(size);
    auryn_vector_float_add_constant(layers[i],-1.);
  }

  for (int i=0; i<2; i++) {
    diffs[i] = gsl_vector_float_calloc(size);
    bufs[i] = gsl_vector_float_alloc(size);
  }

  tr_gxy = new LinearTrace(size,TAUG);
}

void ZynapseConnection::dealloc_vectors()
{
  if (dst->get_post_size() == 0) return;

  for (int i=0; i<3; i++)
    gsl_vector_float_free(layers[i]);

  for (int i=0; i<2; i++) {
    gsl_vector_float_free(diffs[i]);
    gsl_vector_float_free(bufs[i]);
  }
