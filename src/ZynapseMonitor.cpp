...
void ZynapseMonitor::init(ZynapseConnection * source, std::string filename,AurynTime stepsize)
{
...
void ZynapseMonitor::propagate()
{
        if (auryn::sys->get_clock()%ssize==0) {
                AurynDouble mean,std;
                src->g_stats(mean,std);
                outfile << (auryn::sys->get_time()) << " " << src->get_protein() << " " << mean << " "  << std << std::endl;
        }
